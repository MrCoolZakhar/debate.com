...
- `release_delegate_seat(p_code, p_country)` is gated on `is_session_chair` (the `x-chair-suffix` header) and sent through `sessionClient(code, chairSuffix)`. RollCallPanel's two-tap "Free seat" control uses it, on every row (hover-revealed), hidden for a view-only Commenter and on an ended session. Like every chair write, any device holding the chair code can call it; `isViewOnly` hides it, it does not forbid it.
- **Sessions that were live when this shipped had no claims.** The first holder to load (or re-verify) a seat claims it. Two devices already sharing one seat: the first to re-verify keeps it and the other sees "Taken" within 30 s, and the chair frees the seat if the wrong device got it.
...
### Free seat
- The claimed-seat phone icon, its `delegate_seat_availability` fetch and the 20 s poll were removed. The two-tap Free seat control (`release_delegate_seat`) is on EVERY row, revealed on hover/focus (visible while armed), for the Moderator only (`canFreeSeats`: hidden for a Commenter and on an ended session). Releasing a seat nobody holds returns `ok` with `released: 0`.
