...
- `ChatPanel` (`src/components/ChatPanel.tsx`) receives: committee, senderName, isChair, readCounts, onReadCountsChange, readOnly?, onClose? (a close button in the list header), embedded? (no title / close of its own, for the delegate sheet). Pieces live in `src/components/chat/`: `ChatConversationList`, `ChatThread`, `ChatMessageGroup`, `ChatComposer`, `NewGroupSheet`, `ChatAvatar`, `chatTokens`, `ChatDialog`.
...
- Chat, Scoreboard (`[data-tutorial="tab-scoreboard"]`), Motions and Documents all open with `GrowDialog`. Settings (`SettingsPanel`, also on `/voting/[code]`) slides in from the left edge through `src/components/LeftDrawer.tsx`: drag towards the edge to dismiss (a third of the width or a flick), backdrop fade, Escape, focus trap and focus return shared with GrowDialog in `src/components/dialogFocus.ts`. In Arabic it mirrors to the right edge.
...
| Tab | Contents |
|-----|----------|
| Access | session code, chair code, head-chair claim ("Take the gavel"), `requireChairApproval`, `sponsorLabel`, `lockDelegateRollCall`, `disableChat`, `gslRequireNextSpeaker`, Timer sound (`gavelSoundEnabled`, `gavelSoundAtSeconds`, Test sound) |
...
- **No chair can free a seat.** The two-tap "Free seat" control, `releaseDelegateSeat` and the `release_delegate_seat` RPC were removed on 15 Sep 2026 (migration `seat_claims_idle_expiry_leave_and_drop_release`) at Peter's instruction: nobody can kick a delegate out. A seat frees itself instead, through the idle rules below. Do not add a chair release back without being asked.
...
- **Sessions that were live when this shipped had no claims.** The first holder to load (or re-verify) a seat claims it. Two devices already sharing one seat: the first to re-verify keeps it and the other sees "Taken" within 30 s. There is no chair override: a wrongly held seat frees itself 65 minutes after that device goes idle.
...
- Replaced the dark roll-call modal and the side rules console. Full-screen ivory overlay (Portal): a forest summary band (big Present / Present and voting / Absent counts, then plain sentences for what passing needs, computed with `computeVoteOutcome` for "every present delegation votes"), the roll call (round `SeatCircleFlag`, explicit Absent / Present / P+V segments via `setRollCallStatus`, observers apart), the rules (threshold, abstentions, veto, quorum), and ONE primary action, **Start voting**, which confirms the roll call exactly like the old button. Stacks to one column at phone width, RTL via logical properties, entrance animation off under reduced motion.
...
- Everywhere else, including the unmoderated caucus: the speaker holding the floor (#1), then the list in order, then every delegate NOT on the list, A-Z (absent delegates sit in their A-Z place, dimmed).
...
- Round flags through `SeatCircleFlag` (crest, flag, then the unknown-user glyph): 42px, 50px for the speaker at #1, 36px in slider mode (roll call and the Roll Call tab), 5% up from 40/48/34 on 15 Sep 2026, so names still fit at the 288px minimum sidebar width. Names 17px (19.5px for #1, 15.5px in slider mode).
...
- **Status colour is a roll-call thing only** (15 Sep 2026, owner's instruction). In slider mode (pre-session roll call and the mid-session Roll Call tab) Present rows are tinted green and Present-and-Voting gold, and the slider carries the status. Everywhere else Present and Present-and-Voting rows are IDENTICAL: one neutral ivory tint, no PV tag (`statusesMixed`, `rollcall_pv_tag` and `rollcall_pv_full` are gone). Absent rows stay untinted, dimmed, and say `rollcall_absent`.
- **Outside roll call the megaphone is the ONLY per-row control** (15 Sep 2026, owner's instruction): no Observer tag, no slider, just the observer toggle, gold on an observer row and faint (45%, full on row hover / focus) otherwise. The row still shows `rollcall_absent` and the reorder grip, which are not status controls.
- **The megaphone touches the flag** (15 Sep 2026, owner's instruction): a 21px round badge overlapping the flag's bottom inline-end edge (the queue number sits at its top inline-end), inside the flag's `relative` box, not at the row's end. Still its own `<button>` (`aria-pressed`, `rollcall_observer_make` / `rollcall_observer_remove` as title and name, a slightly larger invisible hit area): its click stops propagation and `startPointerDrag` ignores a pointerdown on a button, so it never adds / removes the row or starts a drag. Observer = gold disc, forest glyph; otherwise a forest disc at 45% until the row is hovered (always shown in slider mode); a Commenter or ended session sees a plain gold badge on observer rows only.
- **No X on the speaker's row** (15 Sep 2026, owner's instruction). The floor holder leaves the floor by a click on their row (`onRemoveCurrentSpeaker`), and the floor's top speaker strip keeps its own X. A Commenter or an ended session sees a plain gold megaphone on observer rows only. In slider mode the tag, the toggle and the slider all show, as before. Making an observer mid-session downgrades Present-and-Voting to Present through the parent's `handleStatusChange`, exactly as in roll call.
...
- **Masthead** (`CommitteeIdentityBadge`): an 84px emblem, the name (acronym big, full name beneath), the topic (11.5px, clamped to 3 lines, full text in the tooltip), and the `QuorumRings` tabs flush on its bottom edge (no bottom padding when they show).
...
- **QuorumRings** (`src/components/QuorumRings.tsx`, 15 Sep 2026) are three small **bookmark tabs** growing up out of the speakers list: rounded top corners, concave flares at the foot, in the list's own forest (`ground`). Each has its word on top (`identity_present`, `identity_tab_two_thirds` = "2/3", `identity_tab_majority` = "1/2+1") and a HALF-circle gauge whose arc is filled 100% / 66.7% / 50% (a static pictogram of the threshold, never animated) with the number inside: present, `ceil(present x 2/3)`, `floor(present / 2) + 1`. About 40px tall, which is what moved the list up. Present counts voting delegations only (observers excluded, the same `presentCount` the quorum gate uses). Every cell keeps its full-sentence accessible name (`identity_*_aria`). The three tabs are **spread evenly across the sidebar** (`justify-evenly`, 8px inline padding for the flares; 15 Sep 2026, owner's instruction), so they follow the resizable width. When Settings → Voting → Quorum is set, a compact pill on its OWN line just above the tabs, at the inline end, says "Quorum met" or "Quorum needs N present" (`quorumNeeded = ceil(fraction x total)`), so it never breaks the spacing. The flares use physical left/right because each gradient centre is a physical corner; the arc is mirrored in RTL.
