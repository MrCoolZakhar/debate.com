...
- **`FeedbackLogPanel` is rendered ONLY when `isViewOnly` is true** — the comment dock is a Commenter-exclusive surface, which is exactly what the role is named for. The Moderator runs the room and does not comment. A Moderator toggle for it was built and REMOVED at Peter's instruction; do not add one back without being asked. Because the gate is `isViewOnly`, which is derived from `dbHeadChair` on every committee refetch, taking or handing over the gavel makes the dock appear and disappear on both devices with no reload.
...
- `delegate_seat_availability(p_code, p_token)` returns per seat `{capacity, claimed, full, mine}` with no holder info. The join page disables full seats; `RollCallPanel` shows a phone icon on claimed seats (fetched when the SET of seat countries changes, keyed on a sorted string rather than the `committee.delegates` array so a roll-call tap does not refetch, and every 20 s, since the claims table has no realtime).
- `release_delegate_seat(p_code, p_country)` is gated on `is_session_chair` (the `x-chair-suffix` header) and sent through `sessionClient(code, chairSuffix)`. RollCallPanel's two-tap "Free seat" control uses it, hidden for a view-only Commenter and on an ended session. Like every chair write, any device holding the chair code can call it; `isViewOnly` hides it, it does not forbid it.
