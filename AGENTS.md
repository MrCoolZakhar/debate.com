...
- **Speaker buttons never disappear (15 Sep 2026).** `src/components/SpeakerControls.tsx` renders Restart, Start / Pause, Next (or Finish), Add time (`ClockPlus`, light blue) and Right of Reply (full orange) for the Moderator on the GSL AND the moderated caucus / Tour de Table, with or without a speaker. With the floor empty, Next calls the first delegate (`gsl_call_first`; it carries the tutorial's `call-first-speaker` target, the old big button is gone) and Start / Restart / Add time are `aria-disabled` with a tooltip saying why. Presentational only; every handler is the chair page's.
...
- **Switching later**: the Moderator clicks the topic, in the top bar or the roll-call card's masthead during pre-session, or in the sidebar masthead (`CommitteeIdentityBadge` `onTopicClick`) during debate (the top bar shows tabs, not the topic, once debate starts). Offered only when there are 2+ topics and the session has not ended.
...
- Round flags through `SeatCircleFlag` (crest, flag, monogram): 40px, 48px for the speaker at #1, 34px in slider mode (roll call and the Roll Call tab), so names still fit at the 288px minimum sidebar width. Names 17px (19.5px for #1, 15.5px in slider mode).
...
- **The observer toggle (megaphone) renders only in slider mode.** Outside roll call an observer shows its `rollcall_observer` tag and nothing to change it with; open the Roll Call tab to change it.
...
- **The forest sidebar runs the full height of the screen.** The chair page's root is a flex ROW: `<aside>` + `SidebarResizer`, then a column holding the top bar, the banners and the floor. There is no Gavelling logo on the chair page. `sidebarVisible` keeps the sidebar in exactly the states it used to show in: not in pre-session, not while chat covers the floor, not on the End View / Suspend View tabs.
...
- **Collapsing the sidebar.** The masthead's `PanelLeftClose` button (sidebar only, never the pre-session card) folds the sidebar to a 22px full-height forest rail that IS the reopen button (`PanelLeftOpen`, `sidebar_expand`); Mod+\ toggles it anywhere outside a text field. Persisted per reader like the width (`gavelling-sidebar-collapsed-chair:<name>`, `loadSidebarCollapsed` / `saveSidebarCollapsed` in `src/lib/sidebarWidth.ts`). The floor reflows. Collapsed, the rail carries `data-tutorial="speakers-sidebar"`; expanded, the `<aside>` does. Pre-session has no sidebar (roll call is the centred card), so the state simply waits.
- **SidebarResizer has zero layout width.** An 8px strip centred on the sidebar's edge takes the pointer; a 2px edge rule lights gold and a small forest grip pill (`GripVertical`) appears just OUTSIDE the sidebar on hover, focus or drag, with `pointer-events: none` while invisible so it never swallows a click on the floor. ←/→, Home/End and double-click are unchanged.
- **Masthead** (`CommitteeIdentityBadge`): an 84px emblem, the name (acronym big, full name beneath), the topic (12px), and `QuorumRings` (30px rings, 10px labels).
...
- **QuorumRings** (`src/components/QuorumRings.tsx`) are the old three pies with labels: Present, Two thirds (`ceil(present x 2/3)`), Majority (`floor(present / 2) + 1`). Present counts voting delegations only (observers excluded, the same `presentCount` the quorum gate uses; the old pies counted observers). When Settings → Voting → Quorum is set, a pill says "Quorum met" or "Quorum needs N present" (`quorumNeeded = ceil(fraction x total)`).
