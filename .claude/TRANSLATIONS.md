...
**22 Sep 2026, Sessions resume prompt (not recounted: other sessions are adding keys too):** in all four locales, placed right after `promo_conf_later`, ADDED 26 `srp_*` keys for `src/components/sessions/SessionsResumePrompt.tsx` (the pop-up on /sessions that offers a conference chair their live room and replaces the old hardcoded-English "Session in progress" rejoin card): `srp_conf_eyebrow`, `srp_conf_title` (`{name}`), `srp_conf_title_anon`, `srp_conf_note`, `srp_rejoin_eyebrow`, `srp_rejoin_title`, `srp_chooser_title` (`{n}`), `srp_chooser_hint`, `srp_status_live`, `srp_status_ready`, `srp_status_suspended`, `srp_topic`, `srp_code`, `srp_copy`, `srp_copy_short`, `srp_copied`, `srp_role`, `srp_role_chair`, `srp_role_chair_named` (`{name}`), `srp_join_chair`, `srp_join`, `srp_rejoin`, `srp_not_now`, `srp_dismiss`, `srp_forget`, `srp_close`. No em dashes; French uses the typographic apostrophe. The old card's English strings ("Session in progress", "Signed in as", "Password:", "Rejoin", "Dismiss") are gone with it.
