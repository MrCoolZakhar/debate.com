...
**25 Sep 2026, sessions hero headline (prompt 62, owner):** REWORDED × 4 locales, no key added or removed: `sl_h1_lead` / `sl_h1_accent` are now the site signature "MUN done" / "right." (es "MUN" / "como se debe." since 26 Sep 2026, restoring the earlier Spanish, fr "Le MUN, bien" / "fait.", ar "MUN" / "كما ينبغي."; the accent word is the gold italic one) and `sl_lede` is the two-line tagline "Roll Call, Motions, Voting, Scoring.\nAll in one Session." with a real `\n` in every locale, rendered with `white-space: pre-line` by `SessionsLanding.tsx` so the break is kept. The homepage's SessionsSection says the same in English only (it is not translated, like the rest of the homepage).

**23 Sep 2026, room retention, the record download, phones at roll call (not recounted: other sessions are adding keys too):** in all four locales, placed where `sb_export_csv` was (it is REMOVED: the scoreboard exports an .xlsx record now), ADDED 17 keys: `sb_export_record` (the scoreboard header button), `session_record_download`, `session_record_hint`, `session_record_busy`, `session_record_failed`, `session_record_retry` (the End View's Download the record and the scoreboard button's states), `session_kept_until` (`{when}`, End View), `session_suspended_kept_until` (`{when}`, the chair's Suspend View), `motions_suspend_kept_until` (`{when}`, MotionsModal's Suspend "Does this motion pass?"), and `rc_phones_title`, `rc_phones_yes`, `rc_phones_no`, `rc_phones_join_title`, `rc_phones_scan`, `rc_phones_present`, `rc_phones_change`, `rc_phones_no_note` (`RollCallJoinPanel`). `{when}` is a date and time already formatted by `Intl.DateTimeFormat` in the reader's language. The workbook itself (`sessionRecordExport.ts`) is English on purpose, like the CSV it replaces. Net +16 per locale.

**22 Sep 2026, editing a group chat (not recounted: other sessions are adding keys too):** added 10 keys × 4 locales, placed after `chat_group_hint`: `chat_group_edit` (the header pencil and the sheet title), `chat_group_save`, `chat_group_edit_hint`, `chat_group_edit_failed` (a refused save, the sheet reopens with what was typed), `chat_group_all_in` (nobody left to add), and the thread's system lines `chat_group_event_added` (`{name}`, `{members}`), `chat_group_event_added_you` (`{members}`), `chat_group_event_added_me` (`{name}`: "Alice added you", shown to the member who was added; anyone added with them gets the ordinary line), `chat_group_event_renamed` (`{name}`, `{group}`), `chat_group_event_renamed_you` (`{group}`). The `_you` variants exist so no locale has to conjugate a verb after "You". Reworded `chat_group_info` in all four: a member added later sees the whole history.

**23 Sep 2026, chair walks in without the chair code (not recounted):** in all four locales, in the `srp_*` block, ADDED 8 keys: `srp_start_session`, `srp_join_cochair` (the chair card's primary action, chosen from whether anyone is on the dais), `srp_menu_start` (its short form in the profile menu), `srp_entering` (the button while the room opens), `srp_dais_empty`, `srp_dais_count` (`{n}`), `srp_dais_here` (`{n}`, chairs with the room open right now), `srp_present_count` (`{n}`, `{total}`). REWORDED `srp_status_ready` (en "Not started yet", was "Ready for roll call") so the same line reads correctly for a chair looking at a room nobody has opened. Net +8 per locale.

**22 Sep 2026, "your room is live" pop-up and profile menu Live now (not recounted: other sessions are adding keys too):** in all four locales, placed right after `promo_conf_later`, ADDED 49 `srp_*` keys for `src/components/liveRooms/` (`LiveRoomsDialog`, `LiveNowMenuSection`; the pop-up that sends an organiser, chair, delegate or faculty advisor into their live conference room, and on /sessions replaces the old hardcoded-English "Session in progress" rejoin card): `srp_conf_eyebrow`, `srp_conf_title` (`{name}`), `srp_conf_title_anon`, `srp_conf_note`, `srp_rejoin_eyebrow`, `srp_rejoin_title`, `srp_chooser_title` (`{n}`), `srp_chooser_hint`, `srp_status_live`, `srp_status_ready`, `srp_status_suspended`, `srp_topic`, `srp_code`, `srp_copy`, `srp_copy_short`, `srp_copied`, `srp_role`, `srp_role_chair`, `srp_role_chair_named` (`{name}`), `srp_join_chair`, `srp_rejoin`, `srp_not_now`, `srp_dismiss`, `srp_forget`, `srp_close`, `srp_org_eyebrow`, `srp_org_title` (`{name}`), `srp_org_title_anon`, `srp_org_live_count`, `srp_org_in_session`, `srp_org_open`, `srp_org_rows` (`{live}`, `{session}`), `srp_deleg_title` (`{name}`), `srp_deleg_title_anon`, `srp_deleg_country`, `srp_deleg_note`, `srp_adv_eyebrow`, `srp_adv_title` (`{name}`), `srp_adv_title_anon`, `srp_adv_open`, `srp_adv_note`, `srp_live_eyebrow`, `srp_menu_title`, `srp_menu_join`, `srp_menu_open`, `srp_role_word_chair`, `srp_role_word_delegate`, `srp_role_word_advisor`, `srp_role_word_organiser`. The pop-up renders English outside the sessions routes, like every conferences-side surface. No em dashes; French uses the typographic apostrophe. The old card's English strings ("Session in progress", "Signed in as", "Password:", "Rejoin", "Dismiss") are gone with it.

**18 Sep 2026, languages are a SESSIONS feature + "Request a language" (not recounted: other sessions are adding keys too):** owner: "Remove the languages feature in Gavelling Conferences, just in sessions." `LanguageProvider` now exposes `language` = the stored preference only on a sessions route (`isSessionsPath`, `src/lib/sessionRoutes.ts`: `/create`, `/join`, `/chair`, `/delegate`, `/advisor`, `/voting`, `/sessions`) and `'en'` everywhere else, plus `preferredLanguage` (the stored value, never wiped). So every conferences-side `t()` / `language` read renders English, and `SiteNav` shows its switcher only on `/sessions`. In all four locales, placed right after `settings_french`, ADDED 25 `lang_request_*` keys for `LanguageRequestDialog` (opened from the SiteNav switcher on /sessions, the /create and /delegate menus, the new `SessionLanguageMenu` on /join, and Settings' language picker). No em dashes; French uses the typographic apostrophe.

**18 Sep 2026, voting veto + unanimous result (not recounted: other sessions are adding keys too):** in all four locales, placed right after `voting_pick_status_vetoed`, ADDED 12 keys: `voting_veto_holder` (the pill on the ballot card of a delegation that can veto), `voting_veto_holder_hint` (its tooltip), `voting_veto_confirm_title`, `voting_veto_confirm_body` (`{country}`), `voting_veto_confirm_yes`, `voting_veto_confirm_cancel` (the veto double check on the chair's ballot; the delegate phone reuses the title and the yes key), `voting_veto_flash_sr` (the screen reader line of the red veto flash), `voting_result_unanimous`, `voting_result_unanimous_eyebrow`, `voting_result_unanimous_note`, `voting_result_unanimous_note_abst` (`{n}`), and `dvote_veto_body` (the phone's veto confirmation). French uses the typographic apostrophe. No em dashes. The sideways quorum tabs of the voting roll call reuse the `identity_*` keys of QuorumRings unchanged.

**18 Sep 2026, conference promo pop-up rewritten (not recounted: other sessions are editing keys too):** owner: "way too AI generated. Use bigger sized headings and less words". In all four locales, REWORDED `promo_conf_title` (en "Run your whole conference here.") and `promo_conf_body` (en "Applications, allocations and every committee. Free for organisers."), and REMOVED 7 keys no longer read anywhere: `promo_conf_eyebrow`, `promo_conf_ecosystem_title`, `promo_conf_ecosystem_body`, `promo_conf_promote_title`, `promo_conf_promote_body`, `promo_conf_apps_title`, `promo_conf_apps_body`. `promo_conf_label`, `_cta`, `_later`, `_close`, `_new_tab` are unchanged. Net -7 per locale. On /create, `create_paste_hint` is no longer printed beside Auto-match: it is the paste field's tooltip and its visually hidden description.

**18 Sep 2026, pre-production tutorial + translations pass (counted right after: 1514 keys per locale, identical key sets in all four, 0 missing, 0 orphans, 0 duplicates).**
- **ADDED 49 keys × 4 locales** that replace hardcoded English in the session surfaces. 48 sit in one block at the top of each locale (comment `session surfaces, added 17 Sep 2026`); `caucus_add_first` sits right after `caucus_add_last`. Shared: `session_loading`, `session_signin_title`, `session_signin_chair_body`, `session_signin_btn`, `session_denied_title`, `session_denied_chair_body`, `session_back_home`, `session_try_again`. Chair page: `gsl_already_on_list`, `gsl_enter_hint`, `caucus_currently_speaking`, `caucus_already_spoke`, `caucus_add_first`, `caucus_add_last`, `cow_timer_set_custom`, `caucus_tdt_title` (the caucus loading screen now says جولة المتحدثين in Arabic, the same as the inline ternary elsewhere), `caucus_moderated_title`. Delegate page: `delegate_phase_moderated`, `delegate_phase_unmoderated`, `delegate_phase_voting`, `delegate_phase_adjourned` (Voting Procedure and Debate Closed used to show in English on every phone; the English `PHASE_LABELS` constant is deleted), `delegate_enter_hint`, `delegate_session_not_found_body` (`{code}`), `delegate_language_label`. Voting page: `voting_dismiss`, `voting_signin_title`, `voting_signin_body`, `voting_denied_body`, `voting_not_found_title`, `voting_not_found_body` (`{code}`), `voting_chair_code_wrong`, `voting_chair_gate_title`, `voting_chair_gate_body` (`{code}`), `voting_chair_code_field`, `voting_chair_gate_open`, `voting_chair_gate_no_code` (drops the old em dash and the removed "Go to voting" banner), `voting_rejoin_chair`. Documents: `documents_uploading`, `documents_drop_pdf`, `documents_upload_hint`, `documents_duplicate_title`, `documents_hide_content`, `documents_show_content`, `documents_sponsor_remove` (`{country}`); the attachment "(optional)" reuses `documents_google_docs_optional`. Motions: `motions_error_no_total_time`, `motions_error_no_speaking_time`, `motions_tour_order_custom`; the pencil tooltips reuse `motions_edit_label`. Roll call slider tooltip: `rollcall_slider_hint` (A / P / PV) and `rollcall_slider_hint_observer` (A / P). They use words, not arrows, because arrows read backwards in Arabic.
- **TRANSLATED:** es `join_role_advisor` was still `FACULTY ADVISOR`; it is now `ASESOR ACADÉMICO`. It was the only real untranslated value left in es / fr / ar. Every other value that matches English is legitimately the same word (Quorum, Veto, Caucus, Tour de Table, Photo, Total, PDF, P / PV, language names).
- **REMOVED 216 dead keys × 4 locales (864 lines).** Each key was grepped against the live `src/` tree (as `'key'` and `"key"`) right before deletion, and the checker was tested on control keys first. `t()` only takes a literal `TranslationKey` and the repo has no `as TranslationKey` casts, so no key can be built dynamically. Whole families went: `nav_*`, `conferences_coming_soon` / `conferences_desc*`, `delegate_breakdown_*`, `delegate_tier_*`, `settings_points_tip_*`, `settings_motion_*`, `documents_vote_result_*`, the old `sb_sort_by` / `sb_deduct` / `sb_reason_required` / `sb_manual_adjustment`, `gsl_never_empty_warning`, `gsl_call_first`, `speaker_ctl_call_first*`, `voting_observers_*`, plus leftovers from the create page, pre-vote screen and documents rounds. Seven more keys on the list (`create_conference_*`, `voting_rc_for_doc`, `voting_rights_*`) had already been removed by other sessions. **Trap:** `.claude/worktrees/` holds six old copies of `src/`, and grepping those makes about 215 dead keys look used. Always leave `.claude/worktrees` out when you search for dead keys.
- **Tutorial (`TutorialOverlay.tsx`, inline copy per rule 8, all four locales rewritten together):** a NEW step `seat-add` (after `tab-rollcall`) spotlights the inline seat box through the new anchor `data-tutorial="seat-add"` in `SeatAddField.tsx`. It covers Enter for present and Shift+Enter for observer. The copy was corrected in `call-first-speaker` (the delegation is already on the floor, press Start), `timer` (clicking the clock starts and pauses it), `add-time` (it is the clock icon now, not "+time", and it works before Start), `tab-documents` (a draft resolution goes to the roll call, then the dais or the delegates' phones vote), `tab-settings` (five tabs with People, not four), `join-code` (a click puts the code and a QR code on screen, it does not copy) and the scoreboard card `SB_COPY` (`tune1`: no custom point sources; `earnFoot`: the manual adjustment reason is optional).
- **Still English on purpose (hand-maintained bypasses, unchanged):** the language names in `SettingsPanel.tsx`; "Powered by GIPHY" (`GifPicker`, `ResultBackdrop`); `PDF ≤ 10 MB` on the delegate submit form (a symbol form that reads in any locale); the `defaultName` motion identifiers in `settings/MotionsTab.tsx`; the dead `TYPE_STATIC[].sub` copy in `MotionsModal.tsx` and the two unreachable `FullListPopup` strings in `RollCallPanel.tsx`. Some chair page and MotionsModal strings are still localised with inline `language === 'ar' ? ... : 'fr' ? ...` ternaries (chair page: the add bar placeholder, MODERATED CAUCUS, Unmoderated Caucus, Waiting Room, Custom sec; MotionsModal: the custom-name labels, the topic placeholder, the es/fr default motion names). They reach all four locales but should become keys eventually. The `Loader.tsx` default `label = 'Loading'` (aria only) is outside the session scope and still English.

**22 Sep 2026, voting roll call: Voting bookmark and observer confirmation:** in all four locales, placed where `voting_rc_start_sub` was (after `voting_rc_start`'s neighbours in the `voting_rc_*` block), REMOVED `voting_rc_start_sub` ("Delegations voting: {n}", owner asked for it gone) and ADDED 6 keys: `voting_rc_tab_voting` (the Voting ribbon's label), `device_join_missing_heading` / `device_join_joined_heading` (`{n}`; the two join lists in the Voting drawer, `DeviceVoteGate`), `voting_observer_confirm_title`, `voting_observer_confirm_body_one` / `voting_observer_confirm_body_many` (`{list}`, joined with `Intl.ListFormat` in the reader's language; `ObserverConfirm` in `VotingRollCall.tsx`). Net +5 per locale. The confirmation's buttons reuse `voting_rc_start` and `voting_cancel`.

**17 Sep 2026, create page: conference prompt gone, paste always open:** in all four locales, REMOVED `create_conference_prompt`, `create_conference_login`, `create_conference_open` (-3 per locale). The "Running this as part of a conference? Log in" link was taken off the /create build screen at the owner's instruction, and nothing else read those three keys. `create_paste_toggle` is still read: it is now the visible label above the paste textarea instead of the collapsible button's text, and the paste area is a permanent part of the Delegations section. `create_paste_placeholder`, `create_paste_hint`, `create_auto_match` and the `create_review_*` keys are unchanged. Net -3 per locale.

**17 Sep 2026, device voting review fixes:** ADDED `voting_some_voted` (after `voting_all_voted`, placeholders `{cast}`, `{n}`; the all-voted screen when an early device reveal left delegations out) and `dvote_voted_elsewhere` (after `dvote_not_holder`; the phone when another device already voted for its delegation on that ballot), in all four locales.
...
**17 Sep 2026, create page without the green block:** in all four locales, placed after `create_cta_needs`, ADDED 6 keys: `create_cta_needs_name`, `create_cta_needs_topic` (Start session's sub line when one field is missing; `create_cta_needs` still covers both), `create_cta_sub_later` (sub line with no delegations yet), `create_cta_creating` (the button while the room is created), `create_count_word`, `create_count_word_one` (the word beside the large delegation numeral). No longer read by `src/app/create/page.tsx` (left in place): `create_review_country_tag`, `create_review_custom_tag` (the paste review shows a flag or a person icon instead of a pill), `create_creating`. `create_selected_delegates` is now the screen-reader heading of the roster. Net +6 per locale.
...
**17 Sep 2026, documents polish round (not recounted):** in all four locales, directly after `documents_switch_timings_title`. Added 10 keys × 4 locales: `documents_setup_back` (the setup screen's back arrow, name + tooltip), `documents_finish`, `documents_finish_title`, `documents_finish_confirm`, `documents_finish_outcome_wp`, `documents_finish_outcome_dr`, `documents_finish_now`, `documents_finish_keep` (the Finish tile beside the stage switcher and its question), `documents_no_dr_title`, `documents_no_dr_body` (both take `{doc}` = the committee's draft resolution name; the note shown when Vote has nothing to vote on). `documents_switch_timings` (the word) is no longer read: the Timings key in the timer is icon only and uses `documents_switch_timings_title`. `documents_introduce` is still read by the card's Introduce button.

**18 Sep 2026, Settings round (Points orbit, join-request replica, People dais; not recounted, other sessions were adding keys too):** ADDED `stg_waiting_room` × 4 (directly after `stg_can_join_note`; the Access join card's replica of the chair's Waiting Room bar, same four words as the chair page's inline `wrLabel`, which is still a hand-maintained bypass in `chair/[code]/page.tsx`). REWORDED `stg_can_join_note` × 4 (names the Waiting Room request) and `stg_currently_moderating` / `stg_currently_commenting` × 4 ("(moderating)" / "(commenting)", no "currently"; the key names keep the old word). The join replica now reuses `session_approve` / `session_deny` and no longer reads `delegate_present_btn` / `delegate_pv_btn` (both still read by the delegate page). The People dais no longer reads `gavel_you` (still read elsewhere).
...
### 2026-09-26 — Join: chair role first, roomier seat list (`feature/conferences-auth`) — +15 / -2 keys
- **Added 15 keys × 4 locales**, directly after `join_chair_role_co` (where `join_chair_role_head_note` / `join_chair_role_co_note` were: both **deleted**, the role cards replace them): `chair_role_question` ("How will you chair?"), `chair_role_mod_desc`, `chair_role_mod_rule` ("One Moderator at a time"), `chair_role_com_desc`, `chair_role_com_rule` ("Any number of Commenters"), `chair_role_com_first` ("If nobody is moderating yet, you start with the gavel."), `chair_role_change`, `chair_role_enter`, `chair_role_back`, the takeover confirmation `chair_role_take_title` / `chair_role_take_body` (`{name}`) / `chair_role_take_confirm` / `chair_role_take_cancel`, and the seat list's group headings `join_seat_group_open` / `join_seat_group_closed`.
- Read by `src/components/chairRole/ChairRoleChoice.tsx` (on `/join` and in the "Your room is live" pop-up, which is not a sessions route, so it shows English there), `src/app/join/page.tsx` and `JoinSeatPicker.tsx`. The card titles still read `join_chair_role_head` / `join_chair_role_co` (Moderator / Commenter terminology unchanged).

### 2026-09-24 — Sessions landing redesign + nav labels (`feature/conferences-auth`)
- **Added 36 `sl_*` keys** (43 at first; `sl_have_code` and the six `sl_conf_s*` step keys were dropped the same day) in all four locales for the new `/sessions` landing (`src/app/sessions/landing/*`): hero lede, **Start Committee** (`sl_start`), the smaller join field (`sl_have_code`, `sl_code_placeholder`, `sl_join`), the role tabs (`sl_tab_*`, `sl_chair_*`, `sl_del_*`, `sl_adv_*`, `sl_clip_*`) and the conference section (`sl_conf_*`). The headline itself is not a key: `SessionsLanding.tsx` holds it per language (`HEADLINE`) because the gold accent word is a separate span, exactly as the old page did.
...
- The motion's own name on a History line (and in a delegation's profile) comes from `motionNames()` (renamed or localized), and a Consultation of the Whole segment header uses the same name; no new key. The comment dock's right-of-reply card reuses `sb_hist_right_of_reply`, and its section headers reuse `sb_hist_seg_*`.

### 2026-09-17 — "who has the gavel" notification (`feature/conferences-auth`) — +4 keys
- **Added, 4 keys × 4 locales**, directly after `gavel_device_elsewhere_short` in every locale: `gavel_notify_mine_title` (EN *You have the gavel*, ES *Tienes el mazo*, FR *Vous avez le maillet*, AR *المطرقة معك*), `gavel_notify_mine_body` (*You are moderating this committee now.*), `gavel_notify_taken_title` takes `{name}` (EN *{name} has the gavel*, ES *{name} tiene el mazo*, FR *{name} a le maillet*, AR *المطرقة مع {name}*), `gavel_notify_taken_body` (*They are moderating this committee now.*; FR uses *Cette personne* to stay gender-neutral). Read by the chair page's gavel-holder effect (the glass card in `NotificationStack`). Vocabulary follows the existing *mazo* / *maillet* / *المطرقة*.
- The Commenter floor (`CommenterFloor.tsx`) reuses `gsl_up_next`, `gsl_no_speakers_queued`, `view_is_speaking`, `gsl_on_deck`, `gsl_total` and `commenter_only_hint`; no new keys.

### 2026-09-17 — editable chair comments on the scoreboard (`feature/conferences-auth`) — +4 keys
- **Added 4 keys × 4 locales**, directly after `sb_profile_other_comments`: `sb_note_edit_hint` (the tooltip on a chair's own comment and the line under the open editor: "Click to edit. Enter saves, Escape cancels."), `sb_note_edit_label` (the textarea's accessible name), `sb_note_readonly` (`{name}` — the hover text on another chair's comment), `sb_note_save_failed` ("Not saved. Try again.").
- Read by `src/components/scoreboard/EditableNote.tsx`, which both the History tab and the delegation profile render their comments through. `sb_note_readonly` is the only one with a placeholder: `{name}` is a chair's typed name, never translated.
- The key names for Enter and Escape are left in English inside the sentences (they are the labels on the keys), as elsewhere in the session copy.
