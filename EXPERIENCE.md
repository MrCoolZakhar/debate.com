...
| **M6** | **Room-Order Tour de Table blinds the floor and breaks scoring.** The queue shows `Speaker N` with no name mapping (delegate floor cards go dark), and **speaking time logs to the placeholder country**, so an entire round earns no per-delegate credit. *(15 Sep 2026: the log no longer goes to the placeholder; every delegation present when the tour started is credited one `tour-de-table` speech when it ends, `creditRoomOrderTour`. The blind queue itself is unchanged.)* | log uses `currentSpeaker.country` (= "Speaker N") in `handleNextCaucusSpeaker` | CODE |
