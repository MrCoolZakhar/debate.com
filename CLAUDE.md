...
- **The owner's taste board (25 Sep 2026, 67 votes on real specimens; the board is https://claude.ai/artifact/9CDYVLCgb7h8apgaDPEyad). Overall direction, in the owner's words: "love airbnb!!! literally copy them".** Read these before designing anything; they are votes, not guesses.
  ...
  - **The Explore page (`/conferences/explore`), redesigned 26 Sep 2026 (owner: "revert the background on the explore conferences, doesn't fit the brand ... do a complete redesign of the page. Find best event platforms and do it that way. I like the top bar for searching for sure but the rest can go").** Built after Luma discover, Eventbrite, Meetup and Resident Advisor, in Gavelling's ivory, white, forest and gold. Files: `explore/ConferencesExploreClient.tsx` (every read and filter), `ExploreChrome.tsx` (search pill, filter chips and their popovers, place strip, sort, view toggle), `ExploreFeed.tsx` (month feed, list rows), `exploreFilters.ts` (the pure filters and the URL). Top to bottom:
    ...
    2. **Compact hero, centred.** "Explore Model UN *Conferences*" (heavy sans, `GoldWord` last word), one line under it: the count as a number with the word ("279 conferences across 5 continents") and "Running one? **List your conference**" as a bold underlined link. Then the search pill (kept exactly: Where / When / Role and the round forest search disc; on phones only Where shows). The pill's **When** is the ONE date control (All dates, This week, This month, Next 3 months, Later; This week is `?from=today&to=today+6`, the others `?when=`). Tight spacing: the whole top (hero, pill, one band, results line) fits so the first conference shows above the fold at 1440x900 (owner, 26 Sep 2026: the gap before the conferences must be "one tab max").
    3. **One band: filter chips, then places** (no date tabs row, no Dates chip, no "Browse by Place" heading; owner, 26 Sep 2026: "the dates filter is twice ... filter and by place should be next to each other, then conferences immediately start"). White pill chips, each opening a small popover (a bottom sheet with Clear and Done below 640px): When (phones only, where the pill has no When), Open applications (`?roles=`), Price (`?price=`, "Approximate, converted to USD"), Format (`?format=`, hybrid answers to both), Level (`?level=`), Credit sponsored as an on/off chip (`?sponsored=1`), "Clear all" while anything is on. Then a hairline and the places as chips of the same height (`PlaceStrip`): a small round flag or glyph disc, the name and its count: Everywhere, Near you, the six regions (`?continent=`, one at a time), the top countries plus chosen ones (`?country=`, several), ending in "Another country" (type-to-search). From 1024px the places take the rest of the row and scroll sideways on their own with an edge fade; below that the whole band is one sideways scroll. A custom `?from= ?to=` range in a URL still filters and is named in the pill's When; there is no second date control.
    4. **Results start at once.** The results line (the count as a big number with the words, "43 conferences around the world" / "in Kenya"; "Sort: Soonest first"; the list / grid toggle, list the default, remembered in `localStorage gavelling-explore-view`). Then, when an Explore (or, under a filter, the Region / Country) Spotlight is booked and passes the filters, a small "In the *Spotlight*" label over ONE row of normal listing cards with the gold edge, glow and tag (not repeated in the feed; views and clicks recorded as before; the Conference Spotlight pop-up still opens once per visit). Then the month feed under PLAIN month headers ("October 2026" with "24 conferences"; not sticky, no ground of its own; undated last under "Dates to be announced"). **List** (Luma): each month is one soft 3D white card of rows: date block, cover thumbnail with the logo disc, the name (short form big, full name beneath), round flag + city, dates, format and level, open roles from `explore_conference_facets`, the price in bold with "delegate" in grey, the credit-sponsored heart and "Applied" / "You're in". **Grid**: the photo cards, four columns at 1280, three at 1024, two at 640, one on a phone. Loading is grey skeletons; an empty result is one line and one button.
    5. **The card** (`ConferenceCard variant="listing"`, Explore only; other callers keep the inset white card): a soft 3D white card on the ivory (light top edge, hairline, forest-tinted drop; lifts 3px on hover), the cover photo flush across the top (4:3), the logo disc overlapping its lower edge, then the acronym large with the full name beneath (never "…"), verified check, round flag with city and country, dates, format and delegates as icon + word, price in bold with the role in grey. The whole card is the link. Spotlight keeps the gold edge, glow and tag; credit sponsored keeps the heart.
    6. **Kept exactly.** Every data read and RPC, the URL query parameters (`search, continent, country (repeatable), format, level, roles, price, when, from, to, sort, sponsored`), spotlight view / click recording, the Spotlight pop-up once per visit, Spotlights first, Near you (and the first-load "in your country" / "around you" default), "Bigger Conferences Worth *Travelling*" under a country filter, and the Suspense fallback's real sentence with its plain link to `/conferences/all` (SEO).
  - **No random rectangles (owner, 26 Sep 2026: "you do this mistake a lot, where it's a rectangle randomly. Do not do this").** Never paint a square-cornered, slightly different ivory (or any tinted) band behind a sticky header, a tab row, a filter bar or a section. At rest a sticky header has no background of its own; if it must stay readable over scrolling cards it uses the page's own ground WITH the paper grain (so it is invisible) and fades out at its lower edge. Tinted surfaces are cards: white or soft 3D, rounded, deliberate. Seen on the Explore month headers and the conference page's Overview / You / Reviews bar.
