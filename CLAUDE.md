...
- **The owner's taste board (25 Sep 2026, 67 votes on real specimens; the board is https://claude.ai/artifact/9CDYVLCgb7h8apgaDPEyad). Overall direction, in the owner's words: "love airbnb!!! literally copy them".** Read these before designing anything; they are votes, not guesses.
  ...
  - **The Explore page (`/conferences/explore`), exactly how the owner wants it (26 Sep 2026: "this should be in the design md file clear on how I want it").** Airbnb's search results, in Gavelling's ivory, white, forest and gold. Drawn from the taste boards: the Airbnb search pill with a category rail (YES), a floating filter panel beside the grid (liked), Airbnb-style listing cards that are rectangles with a flag and the conference's own logo on every card, prices in bold with the role in grey. The Airbnb filter MODAL and roomy card rows are rejected.
    1. **Header, small.** "Explore Conferences" in the heavy sans with the last word in the gold Playfair italic (`GoldWord`), and one line under it: the count as a number with the word beside it ("279 conferences across 6 continents"). No eyebrow ("Conference directory"), no "Back to home" link (the nav does that). "List your conference" is the ink outline second button (sentence case) on the right.
    2. **The search pill.** One wide white rounded pill with a soft shadow, Airbnb's: segments Where (country or region), When (dates) and Role (open applications), divided by hairlines, ending in the round forest search disc. It drives the same filters and URL parameters as the panel; nothing new is invented.
    3. **The category rail under it.** A horizontal row of icon-over-word tabs with DUOTONE icons (forest line over a gold or pale green fill, never bare outline glyphs): for example All, Open now, This month, Free, Credit sponsored, In person, Online, University, High school. Each maps onto an existing filter only. The active tab is ink with a short forest underline, like the site nav. It scrolls sideways with round rimmed arrow buttons (the soft rimmed disc the owner loves).
    4. **The floating filter panel.** Beside the grid on desktop: a white card with a soft shadow, sticky, labels above their controls, sentence case, no pressed-in wells, section titles as plain bold words. On phones it becomes a bottom sheet opened by a "Filters" button with the number of active filters as a plain number.
    5. **The results bar.** The count as a big number with the word beside it, the sort as a plain "Sort: Soonest first" text menu, and the grid / list toggle. No count pills.
    6. **The card is an Airbnb listing.** A landscape cover photo on top (about 3:2, radius 16), the conference logo as a round disc overlapping the photo's lower edge, then the text below: the acronym large with the full name smaller beneath (two rows, never "…"), the verified check beside the name, the round flag with city and country, the dates, and the price in bold with the role in grey ("₹2,200 delegate", "Free"). Format is an icon and a plain word (In person, Online), delegates an icon and a number, never uppercase chips. The whole card is the link: no "APPLY" pill. A booked Spotlight keeps its gold edge, glow and "Spotlight" tag; credit sponsored keeps the heart. `ConferenceCard` is this card everywhere it is used.
    7. **Grid.** Four columns beside the panel at 1440, three at 1280, two on a tablet, one on a phone, with generous gaps. Loading is grey skeleton shapes of the same card. An empty result is one line and one button ("Clear filters").
    8. **Keep.** The Conference Spotlight pop-up once per visit, Spotlights first, the Near you country, the URL query parameters, and the Suspense fallback's real sentence with its link to `/conferences/all` (SEO).
