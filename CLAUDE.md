...
- **Redirect-only addresses live in `next.config.ts` `redirects()`** (`/account`, `/account/points` → `/account/manage#credits`, `/account/calendar` → `/account/conferences`, `/account/unlimited`, `/my-conferences`, `/pricing`; `/account/manage` is no longer redirected, it IS the page), not in page files: a server `redirect()` page under the client account layout threw in dev ("'PointsPage' cannot have a negative time stamp"). Next keeps the query, the browser keeps the hash.
- **Where it lives.** `/pricing` → `/pricing/credits`; `/pricing/credits` and `/pricing/subscription` are two tabs in a left rail (public, in the sitemap, linked from the nav as PRICING, the profile menu and every footer); `/help` is the help center (Sessions, Delegates, Chairs, Organizers, Pricing, with search; the Pricing section and both pricing FAQs render the ONE list in `src/lib/pricingFaq.ts` through `src/components/pricing/FaqList.tsx`). "Got a question?" boxes link to `/help`, never to a page's own FAQ. **Manage account is ONE page, `/account/manage` (26 Sep 2026, owner: "There doesn't need to be 3 pages for this"; `src/app/account/manage/ManageAccount.tsx`), in the pricing pages' look:** a hero ("Credits and Plan", See pricing), then `#credits` (the balance as a big number + "credits", Top up opening the credits pop-up, three figures: held, used this year, covered for free; `CreditsSection.tsx`) beside `#subscription` (the plan card, every state above; on Unlimited it is the pricing page's dark glass Unlimited card over `/onboarding/hall-01.jpg` with a gold edge, on Free a white card with the four benefits; `SubscriptionSection.tsx`), then the 30-day usage chart beside `#promo` (a small `claim_promo_code(p_code)` card, the server's `message` shown as returned; `PromoSection.tsx`), then the history from `my_credit_activity()` with the middle column headed "Action". `/account/manage/credits`, `/subscription` and `/promo` are NOT redirected (browsers cached the old permanent 308 from `/account/manage` to `/credits`, so a redirect back would loop): they render the same page scrolled to their section. Links use `/account/manage#credits` / `#subscription` / `#promo`. **My conferences** is `/account/conferences` (26 Sep 2026: the former Conference calendar, renamed; the role-tab "Your conferences" page is archived in `src/app/_archive/account-your-conferences/`): a Next up card with days to go, big-number counts, then at the top what to act on (payments due, imported invitations, drafts to complete, chair and team invitations, from `conferenceActions.tsx`), then Upcoming and Past cards. `/account/calendar` redirects there; `/my-conferences` redirects there; `/account/unlimited` redirects to `/pricing/subscription` keeping its query (old Stripe return URLs); `/account/points` to `/account/manage#credits`. **The account area (26 Sep 2026 redesign)**: one sticky side nav (a raised white card: avatar, name, the plan as a plain word Free / Unlimited with the credits held; Profile, MUN CV, My conferences, Manage account with Credits / Subscription / Promo code as anchors; Sign out) from 1024px, a sideways row of pills below that; every page opens with a white hero band (`AccountHero`, `src/app/account/accountShell.tsx`) whose first cards float over its lower edge (`HeroOverlap`), raised 3D white cards (`RaisedCard`), counts as big numbers with the word beside (`StatBlock`). The MUN CV's "who can see it" is one small Public / Private toggle in its hero with a gear popover for "Show my nationality" (`cv/CVPrivacyPanel.tsx`). The header credit counter is where the coin image appears (and, since 26 Sep 2026, the same pill at the top right of the Import delegates pop-up, `participant/DelegationImportPopup.tsx`, labelled "Your credits"), and it carries a "+" that opens the credits pop-up; the profile menu (`ProfileDropdown`, redesigned 26 Sep 2026: no repetitions) reads: a header (avatar, name wrapping, email, "View profile", the whole block linking to `/account/profile`) with the plan beneath as a plain word, Free or Unlimited, and on Free a gold "Go Unlimited" link that opens the Unlimited pop-up; Live now; Needs your attention; Your conferences (three, then All conferences; the section is absent when there are none); Manage account (the credit balance as a plain number, no "+"); Sign out. No Pricing, About or "Create a conference" rows: the nav, the footer and the header credit counter carry those.
...
- **The footer is information links only** (owner, 23 Sep 2026, after this regressed repeatedly): Explore conferences, roles, List your conference, MUN guides, the session tools, About and contact (one link to /about), and the legal strip. **Never a list of conferences and never a per-conference link in any footer.** Conference pages and country hubs are crawled from `/conferences/all` (`src/app/conferences/all/page.tsx`), which server-renders a real `<a>` for every public conference and every hub and is itself in the sitemap and linked from every footer as ONE "All conferences" link (a link to a directory page, not a list), so the crawl rule above holds without a directory under every page. If a crawl gap appears, fix it on `/conferences/all`.
- **About and contact are one page (26 Sep 2026).** `/contact` 308s to `/about` (`next.config.ts` `redirects()`), and only `/about` is in the sitemap. `/about` (`src/app/about/`) opens with contact (`ContactSection.tsx`: "Talk to Us", three "who are you" cards, organiser / delegate or chair / press, each revealing a name, email, message form in place that posts `{name, email, message, subject}` to `/api/contact` with subject `Conference partnership` / `General enquiry` / `Press and media`; success only on `res.ok`; the email as selectable text with a copy button, and Instagram), then the founders, the ambassadors on a world map (`AmbassadorMap.tsx`), and the ambassador application (the dialog posts to `/api/ambassador` and thanks only on `res.ok`). The map is `public/about/world-dots.svg`, forest dots generated once from `public/map/world_map.png`; its projection and calibration are in `src/app/about/ambassadors.ts`. **To add an ambassador:** add a row to `AMBASSADORS` in that file with a `country` that is a key of `COUNTRY_POINTS` (add the country's capital latitude / longitude there first if it is new; a missing country is listed on phones but left off the map). Photo framing rules are in the same file; `photo: null` draws initials.
...
src/app/
  ...
  (account)    account/{profile,cv,conferences,manage}, auth/*, cv/[id]
  (staff)      admin
...
closing it does. Inbox rows show "Waiting 3 h". A contact-form message (the form at the top of `/about`) that names a
...
- Tokens in `src/components/neu.tsx`: ivory `#EDE7D8` page, `#F0EBDD` surface, forest `#1B3828`, gold `#EED98A`, ink `#1C1410`; shadows are forest-tinted, never neutral; `muted` fails contrast for body text, use `inkSoft`. The typeface is **Inter** everywhere (26 Sep 2026, owner; it replaced Albert Sans, used from 24 Sep, which he found "too weird"; Inter is the closest open face to SF Pro). Outside Sessions every weight reads ONE STEP LIGHTER (900→800, 800→700, 700→600, 600→500; owner, 26 Sep 2026, tried on the dashboard then "do it for the rest"): `<style id="gv-lighter-weights">` built by `LIGHTER_WEIGHTS_SCRIPT` in `src/app/layout.tsx` before first paint, switched off (`media="not all"`) on a Sessions path there and by `SessionsRouteFlag` on client navigation. Sessions pages keep their weights, loaded ONCE by `next/font` in `src/app/layout.tsx` and exposed as `var(--font-brand)`: every `fontFamily` literal reads `"var(--font-brand), sans-serif"`, `font-mono` maps to it too (with tabular figures), and nothing else is loaded except Playfair Display italic for the gold accent word ("MUN done *right.*") and Noto Sans Arabic. Never name a family that is not loaded (the site named Outfit ~870 times without loading it, so every device showed its own system font). Schibsted Grotesk was tried the same day and dropped: its capital I carries serifs. `npm run check:text-overlaps` (`scripts/check-text-overlaps.mjs`, needs a running server, `-- --base=`) reports text that is clipped, truncated or overlapping ONLY with the brand font, against a system-font baseline, at 1440 and 390 wide. Tailwind v4 with no config file.
...
- **Site chrome (25 Sep 2026).** ONE logo: the plain wordmark `public/gavelling-logo.png` through `src/components/BrandLogo.tsx` (`tone` ink / white / gold are CSS filters over that one file; it links to `/`); the "Gavelling Conferences" and "Gavelling Sessions app" lockups are retired (the file was deleted by mistake the day it was added and `BrandLogo` falls back to the old lockup until it is restored; drop the fallback then). ONE footer: `src/components/SiteFooter.tsx` (logo, Instagram, LinkedIn, the © line, then `FooterLegal`, whose links are dark ink and carry Help and one "About and contact" link to /about) on every public page, the account area included; `LabFooter` and every inline footer are gone. The top nav reads, in sentence case, Start / Session (the small word stacked above, → `/sessions`), List your / Conference (→ `/conferences/new`), Explore, Pricing, Help (25 Sep 2026); the active item is ink with a short forest underline, Sign in is the forest gradient button. The `/create` chooser (`src/components/CreateChooser.tsx`) is no longer in the nav; on phones and tablets (below lg) `src/components/MobileTabBar.tsx`, mounted by SiteNav, is an Airbnb-style bottom tab bar: Explore, Join, Create, Conferences, Profile (Log in when signed out), and the page gets bottom padding for it; an Inbox tab waits for a participant inbox page; Home is the logo, About and contact (/about) lives in the footer and the profile menu. ONE gold button: `src/components/GoldButton.tsx` (`GOLD_CTA_*` constants and `.gv-gold-cta`) is every gold call to action, the credits pop-up's PAY button included; `NEU_GRADIENTS.gold` stays for icon discs and progress rings only. The credits pop-up's panel green is `PANEL_GREEN` (`#0F3A28`, `purchaseKit.tsx`), the colour the Stripe Dashboard branding is set to, so the embedded form blends in. The organiser Job Board is gated "Coming soon" (`JOB_BOARD_LIVE` in `manage/[slug]/jobs/page.tsx`) until the public board opens.
...
- **The owner's taste board (25 Sep 2026, 67 votes on real specimens; the board is https://claude.ai/artifact/9CDYVLCgb7h8apgaDPEyad). Overall direction, in the owner's words: "love airbnb!!! literally copy them".** Read these before designing anything; they are votes, not guesses.
  ...
  - **Loved (owner, 26 Sep 2026): the soft round button with a rim, and the bento dashboard.** The round control on a pale disc with a lighter top highlight and a slightly darker outer rim (the organiser dashboard's scroll/arrow disc) is the owner's favourite indicator: "the actual design behind the circle and around". The organiser dashboard's bento of white cards (verified share card, recent activity, the applicants-against-target ring, where applicants come from, participants over time) is "perfect for what we want": use it as the reference for dashboard-like surfaces.
  - **3D cards stay; messages are the glass pop-up (owner, 26 Sep 2026).** "I do love the 3d cards": the raised / pressed neumorphic cards (NeuCard, NEU.out / NEU.in) stay across the organiser tools; this supersedes the taste board's "NOT a raised 3D card". Outcome messages ("Saved", "Couldn't save") are the glass card at the top right, never a banner in the page: `notifyOk` / `notifyErr` (`src/lib/appNotify.ts`, rendered by the manage layout's NotificationStack). Facts that stay true (permission explainers, setup gates, a validation message beside its field, a load failure still retrying) stay in the page, as `appNotify.ts` explains.
  - **The Explore page (`/conferences/explore`), redesigned 26 Sep 2026 (owner: "revert the background on the explore conferences, doesn't fit the brand ... do a complete redesign of the page. Find best event platforms and do it that way. I like the top bar for searching for sure but the rest can go").** Built after Luma discover, Eventbrite, Meetup and Resident Advisor, in Gavelling's ivory, white, forest and gold. Files: `explore/ConferencesExploreClient.tsx` (every read and filter), `ExploreChrome.tsx` (search pill, date tabs, filter chips and their popovers, place rail, sort, view toggle), `ExploreFeed.tsx` (Spotlight row, month feed, list rows), `exploreFilters.ts` (the pure filters and the URL). Top to bottom:
    1. **Ground.** The brand ivory `#EDE7D8` with the site's paper grain (the pricing / legal sheet), never white. White is only for cards, panels and popovers on top of it. `loading.tsx` and the Suspense fallback are ivory too.
    2. **Compact hero, centred.** "Explore Model UN *Conferences*" (heavy sans, `GoldWord` last word), one line under it: the count as a number with the word ("279 conferences across 5 continents") and "Running one? **List your conference**" as a bold underlined link. Then the search pill (kept exactly: Where / When / Role and the round forest search disc; on phones only Where shows, the tabs and chips below carry the rest).
    3. **Date tabs** (Eventbrite / Luma): All dates, This week, This month, Next 3 months, Later, text tabs with a forest underline on the active one. This week is `?from=today&to=today+6` (the existing custom range); the others are `?when=`. A radiogroup, scrolls sideways on phones.
    4. **Filter chips** (Meetup): a slim row of white pill chips, each opening a small popover under it (a bottom sheet with Clear and Done on phones, below 640px): Open applications (roles, `?roles=`), Price (`?price=`, "Approximate, converted to USD"), Dates (custom From / To through the shared DatePicker, `?from= ?to=`), Format (`?format=`, hybrid answers to both), Level (`?level=`), and Credit sponsored as an on/off chip (`?sponsored=1`), then "Clear all" as an underlined link while anything is on. An active chip is a pale green fill with a forest edge and names what is chosen. Popovers use a backdrop for the outside click so a DatePicker calendar (its own Portal, above) works inside them. There is no side panel and no category rail any more.
    5. **Browse by Place.** One sideways row of round rimmed discs (the soft disc the owner loves), each with the name under it (wrapping, never cut) and its count as a plain number: Everywhere, Near you (the profile nationality, else the browser locale's region; round flag and the small "Near you" word), the six regions (`?continent=`, one at a time) and the countries with the most conferences under the other filters plus any chosen ones (round flags, `?country=`, several), ending in "Another country", which opens a type-to-search. The active disc gets a forest ring. Round arrow discs scroll it on desktop.
    6. **In the *Spotlight*.** When an Explore (or, under a filter, the Region / Country) Spotlight is booked and passes the filters, it leads the page as large featured cards: a 16:9 photo, the logo disc over its edge, the name large with the full name beneath, the organiser's booking line (`featured_conferences().description`), flag + city, dates, price, the Spotlight tag and the gold edge and glow. One booking is a wide card (photo left, text right). These conferences are not repeated in the feed. Views and clicks are recorded exactly as before; the Conference Spotlight pop-up still opens once per visit.
    7. **The feed.** A results line (the count as a big number with the words, "43 conferences around the world" / "in Kenya"), the "Sort: Soonest first" text menu and the list / grid toggle (list is the default; the choice is remembered in `localStorage gavelling-explore-view`). The results are grouped by the month they start in, under sticky month headers ("October 2026" with "24 conferences" beside it, ivory, `top: 84px` from lg so the nav pill never covers them; undated conferences last under "Dates to be announced"). **List** (Luma): each month is one soft 3D white card of rows: a date block (month, big day, weekday; tablet and up), a cover thumbnail with the logo disc on its corner (the logo alone on a pale fill when there is no banner), the name (short form big, full name beneath), round flag + city, the dates, format and level as a duotone icon and a word, open roles from `explore_conference_facets` ("Open for delegate, chair" in forest, or "Applications not open"), and the price in bold with "delegate" in grey, the credit-sponsored heart and "Applied" / "You're in". **Grid**: the same months with the photo cards, four columns at 1280, three at 1024, two at 640, one on a phone. Loading is grey skeleton shapes of whichever view; an empty result is one line and one button.
    8. **The card** (`ConferenceCard variant="listing"`, Explore only; other callers keep the inset white card): a soft 3D white card on the ivory (light top edge, hairline, forest-tinted drop; lifts 3px on hover), the cover photo flush across the top (4:3), the logo disc overlapping its lower edge, then the acronym large with the full name beneath (never "…"), verified check, round flag with city and country, dates, format and delegates as icon + word, price in bold with the role in grey. The whole card is the link. Spotlight keeps the gold edge, glow and tag; credit sponsored keeps the heart.
    9. **Kept exactly.** Every data read and RPC, the URL query parameters (`search, continent, country (repeatable), format, level, roles, price, when, from, to, sort, sponsored`), spotlight view / click recording, the Spotlight pop-up once per visit, Spotlights first, Near you (and the first-load "in your country" / "around you" default), "Bigger Conferences Worth *Travelling*" under a country filter, and the Suspense fallback's real sentence with its plain link to `/conferences/all` (SEO).
