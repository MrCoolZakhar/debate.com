...
// The palette, the focus rings, the button skins (sentence case; the gold
// Unlimited skin keeps its capitals), the white section blocks and the
// "Got a question?" close.
...
export function PricingStyles() {
  return (
    <style>{`
      ...
      .gv-p-block{border-radius:28px;padding:clamp(40px,4vw,64px) clamp(24px,4vw,48px)}
      ...
      .gv-p-h2{margin:0;font-size:clamp(24px,2.4vw,40px);font-weight:800;letter-spacing:-0.025em;line-height:1.05;color:${P.ink}}
      ...
      .gv-p-btn{display:inline-flex;align-items:center;justify-content:center;gap:10px;min-height:52px;padding:0 26px;border-radius:12px;font-family:${OUTFIT};font-size:15.5px;font-weight:700;letter-spacing:-0.005em;text-decoration:none;cursor:pointer;border:2px solid transparent;transition:transform 160ms cubic-bezier(0.22,1,0.36,1),background-color 160ms ease-out,box-shadow 160ms ease-out}
      ...
      .gv-p-btn-big{min-height:56px;padding:0 30px;font-size:16.5px}
      .gv-p-btn-forest{background:linear-gradient(90deg,#1B3828 0%,#2A5A3C 55%,#1E4A31 100%);color:#FFFFFF;box-shadow:0 12px 26px -16px rgba(27,56,40,0.7)}
      .gv-p-btn-forest:hover{background:linear-gradient(90deg,#214530 0%,#316946 55%,#245638 100%)}
      .gv-p-btn-gold{text-transform:uppercase;letter-spacing:0.08em;font-size:14px;font-weight:800;background:${GOLD_CTA_BG};color:${GOLD_CTA_INK};box-shadow:${GOLD_CTA_SHADOW}}
      .gv-p-btn-gold:hover{background:${GOLD_CTA_BG_HOVER};box-shadow:${GOLD_CTA_SHADOW_HOVER}}
      /* The gold Unlimited button stays exactly as the owner likes it. */
      .gv-p-btn-gold{border-radius:14px}
      .gv-p-btn-gold.gv-p-btn-big{min-height:60px;padding:0 34px;font-size:15.5px}
      .gv-p-btn-outline{background:transparent;color:${P.ink};border:1.5px solid ${P.ink}}
...
export function QuestionBox() {
  return (
    <aside className="gv-p-question gv-p-open" aria-labelledby="gv-p-question-title">
      ...
      <ActionLink href="/about" skin="forest">Contact us</ActionLink>
    </aside>
  );
}
