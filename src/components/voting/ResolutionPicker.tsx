'use client';

/**
 * ResolutionPicker: the landing of /voting/[code]. Every introduced draft resolution as a
 * card (code, title, sponsors as round flags, a status pill, a live-vote or result line),
 * laid out to fill the page. The Documents "Vote" button lands here; choosing a card that
 * has no vote yet opens the roll call, which starts the vote.
 *
 * It owns no vote logic. `stateOf` is the page's decision for each card (start, resume,
 * view, follow, or nothing a Commenter can open) and `onOpen` hands the choice back.
 *
 * Motion: cards enter with a short stagger (opacity + 8px), lift on hover and settle on
 * press; all of it is off under prefers-reduced-motion.
 */

import type { ReactNode } from 'react';
import { ArrowLeft, ArrowRight, Check, ClipboardList, FileText, Radio, X } from 'lucide-react';
import { useT, useLanguage } from '@/contexts/LanguageContext';
import { SeatCircleFlag } from '@/components/CircleFlag';
import { getCountryDisplayName } from '@/lib/countries';
import type { CommitteeDocument } from '@/lib/types';
import type { VoteStateV1 } from '@/lib/voteState';

const INK = '#1C1410';
const INK_SOFT = '#6A5A4A';
const FOREST = '#1B3828';
const GOLD = '#EED98A';

export type PickCardState =
  | { kind: 'ready'; canOpen: boolean }
  | { kind: 'live'; vote: VoteStateV1; canOpen: boolean }
  | { kind: 'voted'; result: 'passed' | 'failed'; vote: VoteStateV1 | null; canOpen: boolean };

export interface ResolutionPickerProps {
  docs: CommitteeDocument[];
  stateOf: (doc: CommitteeDocument) => PickCardState;
  onOpen: (doc: CommitteeDocument) => void;
  isViewOnly: boolean;
  hideTally: boolean;
  docSingular: string;
  docPlural: string;
  sponsorWord: string;
  /** Moderator: open the roll call without choosing a document. */
  onOpenRollCall?: () => void;
  /** Commenter who stopped following: go back to the live vote. */
  onFollowLive?: () => void;
  onBackToSession: () => void;
  /** Banners, notices: rendered above the heading. */
  children?: ReactNode;
}

function StatusPill({ state }: { state: PickCardState }) {
  const t = useT();
  const base = 'inline-flex items-center gap-1.5 h-7 ps-2 pe-2.5 rounded-full text-[12px] font-black whitespace-nowrap';
  if (state.kind === 'live') {
    return (
      <span className={base} style={{ backgroundColor: FOREST, color: GOLD }}>
        <span className="relative flex w-2 h-2" aria-hidden>
          <span className="gv-pick-ping absolute inset-0 rounded-full" style={{ backgroundColor: GOLD }} />
          <span className="relative w-2 h-2 rounded-full" style={{ backgroundColor: GOLD }} />
        </span>
        {t('voting_pick_status_live')}
      </span>
    );
  }
  if (state.kind === 'voted') {
    const passed = state.result === 'passed';
    return (
      <span className={base} style={{ backgroundColor: passed ? 'rgba(47,107,69,0.13)' : 'rgba(139,32,32,0.10)', color: passed ? '#2F6B45' : '#8B2020' }}>
        {passed ? <Check size={14} strokeWidth={3} aria-hidden /> : <X size={14} strokeWidth={3} aria-hidden />}
        {passed ? t('voting_pick_status_passed') : t('voting_pick_status_failed')}
      </span>
    );
  }
  return (
    <span className={base} style={{ backgroundColor: 'rgba(182,135,31,0.15)', color: '#6A4A0A' }}>
      <Radio size={13} strokeWidth={2.5} aria-hidden />
      {t('voting_pick_status_ready')}
    </span>
  );
}

function Sponsors({ sponsors, word }: { sponsors: string[]; word: string }) {
  const t = useT();
  const { language } = useLanguage();
  if (sponsors.length === 0) {
    return <p className="text-[13px] font-medium" style={{ color: '#9A8A78' }}>{t('voting_pick_no_sponsors')}</p>;
  }
  const shown = sponsors.slice(0, 6);
  const extra = sponsors.length - shown.length;
  const names = sponsors.map((s) => getCountryDisplayName(s, language)).join(language === 'ar' ? '، ' : ', ');
  return (
    <div className="flex items-center gap-3 min-w-0">
      <div className="flex items-center shrink-0 ps-1.5" aria-hidden>
        {shown.map((s, i) => (
          <span key={`${s}-${i}`} className="gv-pick-flag rounded-full -ms-1.5" style={{ zIndex: shown.length - i, boxShadow: '0 0 0 2.5px #FAF8F3' }}>
            <SeatCircleFlag country={s} size={30} decorative />
          </span>
        ))}
        {extra > 0 && (
          <span
            className="-ms-1.5 h-[30px] min-w-[30px] px-1.5 rounded-full flex items-center justify-center text-[11px] font-black tabular-nums"
            style={{ backgroundColor: '#EDE7D8', color: INK_SOFT, boxShadow: '0 0 0 2.5px #FAF8F3' }}
          >
            {t('voting_pick_more', { n: extra })}
          </span>
        )}
      </div>
      <div className="min-w-0">
        <p className="text-[11px] font-black uppercase tracking-[0.12em]" style={{ color: '#9A8A78' }}>{word}</p>
        <p className="text-[13px] font-semibold truncate" style={{ color: INK_SOFT }} title={names}>{names}</p>
      </div>
    </div>
  );
}

function ProgressLine({ value, total, tone }: { value: number; total: number; tone: 'forest' | 'green' | 'red' }) {
  const { language } = useLanguage();
  const pct = total > 0 ? Math.min(1, value / total) : 0;
  const fill = tone === 'forest' ? FOREST : tone === 'green' ? '#2F6B45' : '#8B2020';
  return (
    <div className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: 'rgba(27,56,40,0.10)' }} aria-hidden>
      <div
        className="h-full rounded-full transition-transform duration-500 motion-reduce:transition-none"
        style={{ backgroundColor: fill, transform: `scaleX(${pct})`, transformOrigin: language === 'ar' ? 'right' : 'left' }}
      />
    </div>
  );
}

function Card({ doc, state, index, props }: { doc: CommitteeDocument; state: PickCardState; index: number; props: ResolutionPickerProps }) {
  const t = useT();
  const { isViewOnly, hideTally, onOpen, sponsorWord } = props;
  const action = !state.canOpen
    ? t('voting_pick_not_started')
    : state.kind === 'live'
    ? (isViewOnly ? t('voting_pick_follow') : t('voting_pick_resume'))
    : state.kind === 'voted'
    ? t('voting_pick_view')
    : t('voting_pick_start');

  const vote = state.kind === 'ready' ? null : state.vote;
  const forCount = vote ? vote.votes.filter((v) => v.choice === 'for' || v.choice === 'for-rights').length : 0;
  const againstCount = vote ? vote.votes.filter((v) => v.choice === 'against' || v.choice === 'against-rights').length : 0;
  const live = state.kind === 'live';

  return (
    <button
      type="button"
      onClick={() => { if (state.canOpen) onOpen(doc); }}
      disabled={!state.canOpen}
      className="gv-pick-card gv-pick-in group relative text-start rounded-[28px] p-6 flex flex-col gap-5 min-h-[min(46vh,440px)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#B6871F] focus-visible:ring-offset-2 focus-visible:ring-offset-[#EDE7D8] disabled:cursor-default"
      style={{
        animationDelay: `${Math.min(index, 8) * 70}ms`,
        backgroundColor: '#FAF8F3',
        ['--gv-ring' as string]: live ? 'rgba(182,135,31,0.55)' : 'rgba(27,56,40,0.07)',
      }}
    >
      <div className="flex items-center justify-between gap-3">
        <span className="inline-flex items-center gap-2 h-8 ps-2 pe-3 rounded-full text-[13px] font-black tabular-nums" style={{ backgroundColor: 'rgba(27,56,40,0.08)', color: FOREST }}>
          <span className="w-5 h-5 rounded-full flex items-center justify-center" style={{ backgroundColor: FOREST, color: GOLD }} aria-hidden>
            <FileText size={11} strokeWidth={2.75} />
          </span>
          {doc.docCode}
        </span>
        <StatusPill state={state} />
      </div>

      <h2 className="text-[22px] leading-[1.18] font-black line-clamp-3 [text-wrap:balance]" style={{ color: INK }}>
        {doc.title || doc.docCode}
      </h2>

      <Sponsors sponsors={doc.sponsors} word={sponsorWord} />

      <div className="mt-auto flex flex-col gap-3">
        {live && vote && (
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between text-[12px] font-bold tabular-nums" style={{ color: INK_SOFT }}>
              <span>{t('voting_voted_of', { cast: vote.votes.length, total: vote.order.length })}</span>
            </div>
            <ProgressLine value={vote.votes.length} total={vote.order.length} tone="forest" />
          </div>
        )}
        {state.kind === 'voted' && vote && !hideTally && (
          <div className="flex flex-col gap-1.5">
            <span className="text-[12px] font-bold tabular-nums" style={{ color: INK_SOFT }}>
              {t('voting_pick_tally', { for: forCount, against: againstCount })}
            </span>
            <ProgressLine value={forCount} total={forCount + againstCount} tone={state.result === 'passed' ? 'green' : 'red'} />
          </div>
        )}
        <div
          className="flex items-center justify-between gap-3 pt-4"
          style={{ boxShadow: 'inset 0 1px 0 rgba(27,56,40,0.08)' }}
        >
          <span className="text-[15px] font-black" style={{ color: state.canOpen ? FOREST : '#9A8A78' }}>{action}</span>
          {state.canOpen && (
            <span
              className="gv-pick-arrow w-10 h-10 rounded-full flex items-center justify-center shrink-0"
              style={{ backgroundColor: live || state.kind === 'ready' ? FOREST : 'rgba(27,56,40,0.09)', color: live || state.kind === 'ready' ? GOLD : FOREST }}
              aria-hidden
            >
              <ArrowRight size={18} strokeWidth={2.5} className="gv-pick-arrow-icon" />
            </span>
          )}
        </div>
      </div>
    </button>
  );
}

export function ResolutionPicker(props: ResolutionPickerProps) {
  const t = useT();
  const { language } = useLanguage();
  const { docs, stateOf, isViewOnly, docSingular, docPlural, onOpenRollCall, onFollowLive, onBackToSession, children } = props;
  const states = docs.map((d) => stateOf(d));
  const counts = {
    ready: states.filter((s) => s.kind === 'ready').length,
    live: states.filter((s) => s.kind === 'live').length,
    passed: states.filter((s) => s.kind === 'voted' && s.result === 'passed').length,
    failed: states.filter((s) => s.kind === 'voted' && s.result === 'failed').length,
  };
  const cols = Math.min(Math.max(docs.length, 1), 3);

  return (
    <div className="gv-pick flex-1 min-h-0 overflow-y-auto" dir={language === 'ar' ? 'rtl' : undefined} style={{ ['--gv-dir' as string]: language === 'ar' ? -1 : 1 }}>
      <style>{`
        @keyframes gvPickIn { from { opacity: 0; transform: translateY(10px) } to { opacity: 1; transform: none } }
        @keyframes gvPickPing { 0% { transform: scale(1); opacity: .75 } 80%, 100% { transform: scale(2.4); opacity: 0 } }
        .gv-pick .gv-pick-in { animation: gvPickIn 420ms cubic-bezier(0.2,0,0,1) backwards }
        .gv-pick .gv-pick-ping { animation: gvPickPing 1.6s cubic-bezier(0,0,0.2,1) infinite }
        .gv-pick .gv-pick-card {
          box-shadow: 0 0 0 1px var(--gv-ring), 0 1px 2px rgba(27,56,40,0.06), 0 10px 28px rgba(27,56,40,0.08);
          transition: transform 220ms cubic-bezier(0.2,0,0,1), box-shadow 220ms cubic-bezier(0.2,0,0,1);
        }
        .gv-pick .gv-pick-card:not(:disabled):hover {
          transform: translateY(-4px);
          box-shadow: 0 0 0 1px var(--gv-ring), 0 2px 4px rgba(27,56,40,0.08), 0 22px 44px rgba(27,56,40,0.14);
        }
        .gv-pick .gv-pick-card:not(:disabled):active { transform: translateY(-1px) scale(0.99); transition-duration: 90ms }
        .gv-pick .gv-pick-card:disabled { opacity: .72 }
        .gv-pick .gv-pick-arrow { transition: transform 220ms cubic-bezier(0.2,0,0,1) }
        .gv-pick .gv-pick-arrow-icon { transform: scaleX(var(--gv-dir, 1)) }
        .gv-pick .gv-pick-card:not(:disabled):hover .gv-pick-arrow { transform: translateX(calc(var(--gv-dir, 1) * 4px)) }
        .gv-pick .gv-pick-flag { transition: transform 200ms cubic-bezier(0.2,0,0,1) }
        .gv-pick .gv-pick-card:not(:disabled):hover .gv-pick-flag { transform: translateY(-2px) }
        @media (prefers-reduced-motion: reduce) {
          .gv-pick .gv-pick-in, .gv-pick .gv-pick-ping { animation: none }
          .gv-pick .gv-pick-card, .gv-pick .gv-pick-arrow, .gv-pick .gv-pick-flag { transition: none }
          .gv-pick .gv-pick-card:not(:disabled):hover, .gv-pick .gv-pick-card:not(:disabled):active { transform: none }
          .gv-pick .gv-pick-card:not(:disabled):hover .gv-pick-arrow, .gv-pick .gv-pick-card:not(:disabled):hover .gv-pick-flag { transform: none }
        }
      `}</style>
      {children}
      <div className="mx-auto w-full max-w-[1360px] px-4 sm:px-8 pt-8 pb-12">
        <div className="gv-pick-in flex flex-col lg:flex-row lg:items-end gap-5 lg:gap-8 mb-7">
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-black uppercase tracking-[0.18em]" style={{ color: '#8A6414' }}>{t('voting_pick_eyebrow')}</p>
            <h1 className="text-[32px] sm:text-[40px] font-black leading-[1.05] mt-1 [text-wrap:balance]" style={{ color: INK }}>
              {t('voting_pick_title', { doc: docSingular.toLowerCase() })}
            </h1>
            <p className="text-[16px] mt-2 [text-wrap:pretty]" style={{ color: INK_SOFT }}>
              {isViewOnly ? t('voting_pick_sub_follow') : t('voting_pick_sub')}
            </p>
          </div>
          {docs.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap">
              {counts.live > 0 && <Count icon={<Radio size={14} strokeWidth={2.5} />} n={counts.live} label={t('voting_pick_status_live')} tone="forest" />}
              <Count icon={<FileText size={14} strokeWidth={2.5} />} n={counts.ready} label={t('voting_pick_status_ready')} tone="gold" />
              {counts.passed > 0 && <Count icon={<Check size={14} strokeWidth={3} />} n={counts.passed} label={t('voting_pick_status_passed')} tone="green" />}
              {counts.failed > 0 && <Count icon={<X size={14} strokeWidth={3} />} n={counts.failed} label={t('voting_pick_status_failed')} tone="red" />}
              {onOpenRollCall && (
                <button
                  type="button"
                  onClick={onOpenRollCall}
                  className="inline-flex items-center gap-2 h-11 ps-3.5 pe-4 rounded-full text-[14px] font-black focus:outline-none focus-visible:ring-2 focus-visible:ring-[#B6871F] transition-[background-color,transform] duration-150 active:scale-[0.96] motion-reduce:transition-none"
                  style={{ backgroundColor: '#FAF8F3', color: FOREST, boxShadow: '0 0 0 1px rgba(27,56,40,0.12), 0 2px 6px rgba(27,56,40,0.08)' }}
                >
                  <ClipboardList size={17} strokeWidth={2.25} aria-hidden />
                  {t('voting_roll_call_heading')}
                </button>
              )}
              {onFollowLive && (
                <button
                  type="button"
                  onClick={onFollowLive}
                  className="inline-flex items-center gap-2 h-11 ps-3.5 pe-4 rounded-full text-[14px] font-black focus:outline-none focus-visible:ring-2 focus-visible:ring-[#B6871F] transition-transform duration-150 active:scale-[0.96] motion-reduce:transition-none"
                  style={{ backgroundColor: FOREST, color: GOLD }}
                >
                  <Radio size={16} strokeWidth={2.5} aria-hidden />
                  {t('voting_follow_live')}
                </button>
              )}
            </div>
          )}
        </div>

        {docs.length === 0 ? (
          <div className="gv-pick-in mx-auto max-w-xl rounded-[28px] px-8 py-12 text-center flex flex-col items-center" style={{ backgroundColor: '#FAF8F3', boxShadow: '0 0 0 1px rgba(27,56,40,0.07), 0 10px 28px rgba(27,56,40,0.08)' }}>
            <span className="w-16 h-16 rounded-full flex items-center justify-center mb-5" style={{ backgroundColor: FOREST, color: GOLD, boxShadow: '0 6px 18px rgba(27,56,40,0.25)' }} aria-hidden>
              <FileText size={28} strokeWidth={2.25} />
            </span>
            <h2 className="text-[24px] font-black leading-tight [text-wrap:balance]" style={{ color: INK }}>
              {t('voting_pick_empty_title', { doc: docSingular.toLowerCase() })}
            </h2>
            <p className="text-[15px] mt-2 max-w-md [text-wrap:pretty]" style={{ color: INK_SOFT }}>{t('voting_pick_empty_body')}</p>
            <button
              type="button"
              onClick={onBackToSession}
              className="mt-7 inline-flex items-center gap-2 h-12 px-6 rounded-full text-[15px] font-black focus:outline-none focus-visible:ring-2 focus-visible:ring-[#B6871F] focus-visible:ring-offset-2 transition-transform duration-150 active:scale-[0.96] motion-reduce:transition-none"
              style={{ backgroundColor: FOREST, color: GOLD, boxShadow: '0 2px 4px rgba(27,56,40,0.2), 0 10px 24px rgba(27,56,40,0.2)' }}
            >
              <ArrowLeft size={17} strokeWidth={2.25} aria-hidden style={{ transform: language === "ar" ? "scaleX(-1)" : undefined }} />
              {t('voting_hdr_back_title')}
            </button>
          </div>
        ) : (
          <div
            className={`grid gap-5 sm:gap-6 grid-cols-1 ${cols >= 2 ? 'md:grid-cols-2' : ''} ${cols >= 3 ? 'xl:grid-cols-3' : ''} ${cols === 1 ? 'max-w-3xl' : ''}`}
            aria-label={docPlural}
          >
            {docs.map((doc, i) => <Card key={doc.id} doc={doc} state={states[i]} index={i + 1} props={props} />)}
          </div>
        )}
      </div>
    </div>
  );
}

function Count({ icon, n, label, tone }: { icon: ReactNode; n: number; label: string; tone: 'forest' | 'gold' | 'green' | 'red' }) {
  const palette = {
    forest: { bg: FOREST, fg: GOLD },
    gold: { bg: 'rgba(182,135,31,0.15)', fg: '#6A4A0A' },
    green: { bg: 'rgba(47,107,69,0.13)', fg: '#2F6B45' },
    red: { bg: 'rgba(139,32,32,0.10)', fg: '#8B2020' },
  }[tone];
  return (
    <span className="inline-flex items-center gap-2 h-11 ps-3 pe-4 rounded-full text-[14px] font-bold" style={{ backgroundColor: palette.bg, color: palette.fg }}>
      <span aria-hidden>{icon}</span>
      <span className="font-black tabular-nums">{n}</span>
      <span>{label}</span>
    </span>
  );
}
