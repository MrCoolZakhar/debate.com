...
export default function ChatPanel({ committee, senderName, isChair = false, onClose, speakerCard }: {
  ...
  const conversations = useMemo<Conversation[]>(() => {
    ...
    // "Chairs" group thread (delegates only):
    //   outgoing = sent to 'Chairs', incoming = any chair's reply to this delegate
  ...
  const displayedConversations = useMemo(() => {
    ...
    const fixed = conversations.filter((c) => c.key === 'everyone' || c.key === 'chairs');
    const dynamic = conversations.filter((c) => c.key !== 'everyone' && c.key !== 'chairs');
  ...
  const displayMessages = useMemo(() => {
    const relevant = localMessages.filter((m) => {
      ...
      // Bilateral exchange with activeConv partner
      return m.isPrivate &&
  ...
  useEffect(() => {
    ...
    setReadCounts((prev) => {
      if ((prev[activeConv] ?? 0) >= received) return prev;
      return { ...prev, [activeConv]: received };
    });
  }, [activeConv, displayedConversations, senderName]);
  ...
  const handleSend = () => {
    ...
    setMsg('');
    // Sending a message makes a draft thread permanent
    if (draftThread?.key === activeConv) setDraftThread(null);
    const isPrivate = activeConv !== 'everyone';
    const recipient =
      activeConv === 'everyone' ? undefined :
      activeConv === 'chairs' ? 'Chairs' :
      activeConv;
        ...
        <div className="px-4 pb-4 pt-3 shrink-0 border-t border-[#2E1E0F]">
          <div className="flex gap-2">
            <input
              ...
              placeholder={
                activeConv === 'everyone' ? 'Message everyone…' :
                activeConv === 'chairs' ? 'Message chairs…' :
                `Message ${activeConv}…`
              }
