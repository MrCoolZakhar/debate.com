...
import {
  ...
  addDelegate as addDelegateInDB,
...
function AddCountryInput({ committee, onAdd }: { committee: Committee; onAdd: (country: string) => void }) {
  ...
  const commit = (name: string) => {
    if (!existingNames.has(name.toLowerCase())) onAdd(name);
    setQuery('');
      ...
      {query && allMatches.length > 0 && (
        <div className="absolute bottom-full left-0 right-0 mb-1 bg-[#150F09] border border-[#2E1E0F] rounded-xl overflow-hidden z-30 shadow-xl max-h-52 overflow-y-auto">
          {allMatches.slice(0, 8).map((c, i) => {
            ...
            return (
              <button key={c.code} onMouseDown={(e) => { e.preventDefault(); if (!alreadyAdded) commit(c.name); }}
                disabled={alreadyAdded}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left transition-colors ${alreadyAdded ? 'opacity-50 cursor-default bg-[#2E1E0F]' : i === 0 ? 'bg-[#7B4A1E]/20 text-white' : 'text-[#E8D5B7] hover:bg-[#2E1E0F]'}`}>
                <span className="text-base">{getFlagEmoji(c.code)}</span>
                <span className="text-sm flex-1">{c.name}</span>
                {alreadyAdded ? <span className="text-[10px] text-yellow-500 shrink-0">Already added</span>
                  : i === 0 ? <span className="text-[10px] text-[#7A5A38] shrink-0">Enter ↵</span> : null}
              </button>
            );
          })}
        </div>
...
export default function RollCallPanel({
  ...
  // Optimistic: update local state instantly, then sync to DB
  ...
  const handleAddDelegate = (country: string) => {
    ...
    addDelegateInDB(committee.id, country);
  };
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
        {filtered.map((d) => {
          ...
          return (
            <div key={d.id} onClick={onAddToList ? () => onAddToList(d.id) : undefined}
              className={`flex items-center gap-2 px-2.5 py-2 rounded-xl transition-all ${
                d.status === 'present' ? 'bg-green-950/30 border border-green-800/30'
                : d.status === 'present-voting' ? 'bg-blue-950/30 border border-blue-800/30'
              ...
              <span className={`flex-1 text-sm truncate ${d.status !== 'absent' ? 'text-white font-medium' : 'text-[#7A5A38]'}`}>{d.country}</span>
