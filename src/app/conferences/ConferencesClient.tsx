...
export default function ConferencesClient() {
  ...
  useEffect(() => {
    async function fetchConferences() {
      ...
      const { data } =
        activeTab === 'yours' && user
          ? await (base as any).eq('organizer_id', user.id)
          : await (base as any).eq('is_public', true);
    ...
    <div className="min-h-screen flex flex-col relative" style={{ backgroundColor: '#EDE7D8' }}>
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        >
          ...
          <button
            onClick={() => {
              if (!user) router.push('/auth/signin?next=/conferences/new');
              else router.push('/conferences/new');
            }}
        ...
        {user && (
          <div
            className="px-6 md:px-14 flex gap-6"
            style={{ backgroundColor: '#FAF8F3', borderBottom: '1px solid #DDD4C0' }}
          >
            {(['all', 'yours'] as const).map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className="py-3 text-sm font-bold transition-colors focus:outline-none"
                style={{
                  color: activeTab === tab ? '#1C1410' : '#9A8A78',
                  fontFamily: "'Outfit', sans-serif",
                  letterSpacing: '0.04em',
                  background: 'none',
                  border: 'none',
                  borderBottom: activeTab === tab ? '2px solid #1B3828' : '2px solid transparent',
                  cursor: 'pointer',
                }}
              >
                {tab === 'all' ? 'ALL CONFERENCES' : 'YOUR CONFERENCES'}
              </button>
            ))}
          </div>
        )}
