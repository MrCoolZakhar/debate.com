...
// Manage account, the promo code card (one page since 26 Sep 2026, see
// ManageAccount.tsx). A code field, a Claim button that appears with the
// first character, and on success the
...
import { InfoTip, PrimaryButton, WhiteCard, FOREST, FOREST_MID, INK } from './manageUi';
...
/** The promo code card of Manage account. */
export default function PromoSection() {
  ...
  return (
    <WhiteCard id="promo" style={{ scrollMarginTop: 96, padding: 24 }}>
      <style>{`
      ...
      <div className="flex items-center gap-3 mb-4">
        <span aria-hidden className="inline-flex items-center justify-center flex-shrink-0 rounded-[14px]" style={{ width: 42, height: 42, background: 'linear-gradient(135deg, rgba(238,217,138,0.45), rgba(238,217,138,0.22))', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.7), 0 3px 8px -3px rgba(27,56,40,0.22)' }}>
          <Emoji3D name="Ticket" size={28} />
        </span>
        <h2 className="flex items-center gap-2" style={{ margin: 0, fontFamily: OUTFIT, fontWeight: W.title, fontSize: 20, letterSpacing: '-0.01em', color: INK }}>
          Promo <GoldWord>Code</GoldWord>
          <InfoTip text={INFO} />
        </h2>
      </div>
          ...
          >
            ...
            <input
              ...
              style={{
                ...
                borderRadius: 18,
                boxShadow: focused
                  ? `inset 2px 2px 5px rgba(27,56,40,0.10), inset -2px -2px 5px rgba(255,255,255,0.8), 0 0 0 2px ${FOREST}`
                  : 'inset 2px 2px 5px rgba(27,56,40,0.10), inset -2px -2px 5px rgba(255,255,255,0.8), 0 0 0 1px rgba(27,56,40,0.10)',
                fontFamily: OUTFIT,
            ...
            {trimmed.length > 0 && (
              <div className="gv-promo-rise mt-4">
                <PrimaryButton type="submit" disabled={busy} height={48} className="w-full">
                  {busy ? 'Checking…' : 'Claim'}
                </PrimaryButton>
              </div>
            )}
