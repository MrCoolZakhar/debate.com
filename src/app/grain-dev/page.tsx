...
export default function GrainDevPage() {
  ...
  const bg = isDark ? '#0D0906' : '#F5EFE7';
  const textPrimary = isDark ? '#FFFFFF' : '#1A0E06';
  const textMuted = isDark ? '#C4A882' : '#7B4A1E';
  const textFaint = isDark ? '#7A5A38' : '#B8996A';
  const border = isDark ? '#2E1E0F' : '#D4C4B0';
  const cardBg = isDark ? '#1A1209' : '#EDE3D6';
  const navBg = isDark ? '#150F09' : '#EAE0D2';
  ...
  function LandingHero({ darkMode }: { darkMode: boolean }) {
    const bgColor = darkMode ? '#0D0906' : '#F5EFE7';
    const txt = darkMode ? '#FFFFFF' : '#1A0E06';
    const muted = darkMode ? '#C4A882' : '#7B4A1E';
    const faint = darkMode ? '#7A5A38' : '#B8996A';
    const bd = darkMode ? '#2E1E0F' : '#D4C4B0';
    const card = darkMode ? '#1A1209' : '#EDE3D6';
    const nav = darkMode ? '#150F09' : '#EAE0D2';
      ...
      <div className="relative overflow-hidden flex flex-col" style={{ backgroundColor: bgColor, minHeight: showSideBySide ? '100vh' : undefined }}>
        ...
        <section className="relative z-10 flex-1 flex flex-col items-center justify-center py-16 px-4 text-center">
          ...
          <h1 className="text-5xl md:text-6xl font-black tracking-tight leading-[1.05] mb-5" style={{ color: txt }}>
            ...
            <span style={{ color: '#B8844A' }}>with Confidence</span>
          </h1>
          ...
          <div className="px-10 py-5 rounded-2xl font-black text-xl text-white mb-6 shadow-lg"
            style={{ backgroundColor: '#7B4A1E', boxShadow: '0 8px 24px rgba(123,74,30,0.25)' }}>
          ...
          <div className="w-full max-w-xs">
            ...
            <div className="flex gap-2">
              <div className="flex-1 rounded-xl px-4 py-3 text-sm font-mono text-center" style={{ backgroundColor: darkMode ? '#150F09' : '#E8DDD0', border: `1px solid ${bd}`, color: faint }}>
                Session code
              </div>
              <div className="px-4 py-3 rounded-xl font-bold text-sm" style={{ backgroundColor: darkMode ? '#2E1E0F' : '#D4C4B0', border: `1px solid ${bd}`, color: muted }}>
                Join →
              </div>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#0a0a0a' }}>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex items-center gap-4 px-4 py-2 text-xs"
        ...
        <Link href="/" className="text-[#7B4A1E] hover:text-[#C4A882] font-bold mr-2">← back</Link>
        ...
        {!showSideBySide && (
          <label className="flex items-center gap-2 cursor-pointer select-none">
            ...
            <button
              ...
              className={`relative w-10 h-5 rounded-full transition-colors ${isDark ? 'bg-[#7B4A1E]' : 'bg-[#C4A882]'}`}
            >
        ...
        <label className="flex items-center gap-1.5 cursor-pointer select-none">
          <input type="checkbox" checked={showSideBySide} onChange={(e) => setShowSideBySide(e.target.checked)} className="accent-[#7B4A1E]" />
        ...
        <label className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-[#222] border border-[#444] rounded px-1.5 py-0.5 text-white text-xs"
          >
        ...
        <button
          ...
          className="ml-auto px-2 py-1 rounded text-[#7B4A1E] hover:text-[#C4A882] border border-[#333] transition-colors"
        >
          Copy CSS
        </button>
      </div>
