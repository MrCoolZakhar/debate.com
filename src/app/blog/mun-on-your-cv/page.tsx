  ...
  return (
    <>
      ...
      >
        ...
        <p>That last one is what the MUN CV on <Link href="/">Gavelling</Link> is for. Every profile resolves to a public page you can put on an application, listing conferences, roles and awards, and the entries fall into two kinds: ones you added yourself, and ones written by the conference itself, which carry a verified mark. The distinction is the point. A self-reported entry is a claim, and a conference-issued one is a record, and a reader can see which is which on the page.</p>
        <p>Two honest caveats. Verified entries only exist where the conference itself is running on the platform and has issued them, so most of an older delegate&apos;s history will remain self-reported, and that is fine: an unverified entry is not a suspect one, it is simply unconfirmed. And the awards side of the organiser tools is still being built, so if you are a delegate wondering why your conference has not issued anything, that is why.</p>
